...
<html lang="en">
<head>
    <title>Code coverage report for marko-v4/node-require.js</title>
    <meta charset="utf-8" />
...
<div class='wrapper'>
  <div class='pad1'>
    <h1>
      <a href="../index.html">all files</a> / <a href="index.html">marko-v4/</a> node-require.js
    </h1>
    <div class='clearfix'>
...
&nbsp;
    <span class="missing-if-branch" title="else path not taken" >E</span>if (compilerOptions) {
        compilerOptions = markoCompiler.defaultOptions;
    } else {
<span class="cstat-no" title="statement not covered" >        compilerOptions = Object.assign({}, markoCompiler.defaultOptions, compilerOptions);</span>
    }
...
&nbsp;
        <span class="missing-if-branch" title="if path not taken" >I</span>if (markoCompiler.defaultOptions.assumeUpToDate &amp;&amp; <span class="branch-1 cbranch-no" title="branch not covered" >fs.existsSync(targetFile))</span> {
            // If the target file already exists and "assumeUpToDate" then just use the previously
            // compiled template.
<span class="cstat-no" title="statement not covered" >            return fs.readFileSync(targetFile, fsReadOptions);</span>
        }
&nbsp;
...
&nbsp;
        <span class="missing-if-branch" title="if path not taken" >I</span>if (isUpToDate) {
<span class="cstat-no" title="statement not covered" >            compiledSrc = fs.readFileSync(targetFile, fsReadOptions);</span>
        } else {
            templateSrc = fs.readFileSync(templatePath, fsReadOptions);
...
function getLoadedTemplate(path) {
    ...
    return cached &amp;&amp; cached.exports.render ? <span class="branch-0 cbranch-no" title="branch not covered" >cached.exports </span>: undefined;
}
&nbsp;
...
&nbsp;
    <span class="missing-if-branch" title="if path not taken" >I</span>if (extension.charAt(0) !== '.') {
<span class="cstat-no" title="statement not covered" >        extension = '.' + extension;</span>
    }
&nbsp;
...
&nbsp;
    require.extensions[extension] = function markoExtension(module, filename) {
        ...
        <span class="missing-if-branch" title="if path not taken" >I</span>if (cachedTemplate) {
            // The template has already been loaded so use the exports of the already loaded template
<span class="cstat-no" title="statement not covered" >            module.exports = cachedTemplate;</span>
